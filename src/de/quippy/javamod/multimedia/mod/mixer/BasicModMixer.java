...
public abstract class BasicModMixer
{
	protected class ChannelMemory
	{
		...
		{
			panning = 128; // 0-256, this is therefor center
			actRampVolLeft = 
			actRampVolRight =
			deltaVolLeft =
			deltaVolRight =
	...
	protected boolean doFastForward; // will result in no mixing of samples
	...
	public PlayThreadEventListener listener;
	public PlayThread listenerThread;
...
//		}
	...
	protected void informListener()
	{
		if (listener != null )
		{
			PatternRow row = currentPattern.getPatternRow(currentRow);
			this.listener.mixerEventOccured(currentRow, row);
			/*PatternElement[] el = row.getPatternElement();
			for (int i=0; i<el.length; i++)
			{
				int rowIndex = currentRow;
				int channel = i;
				int sample = el[i].getInstrument();
