...
public class ScreamTrackerMixer extends BasicModMixer
{
	...
	protected void initializeMixer(int channel, ChannelMemory aktMemo)
	{
		if (mod.getModType()==Helpers.MODTYPE_IT)
		{
			...
			aktMemo.doSurround = (mod.getPanningValue(channel) == 400); // 100<<2
		}
	}
	/**
	...
	protected void doRowEffects(ChannelMemory aktMemo)
	{
		...
		switch (aktMemo.effekt)
		{
			...
			case 0x04 :			// Volume Slide
				...
				if (((aktMemo.volumSlideValue>>4)==0xF && (aktMemo.volumSlideValue&0xF)!=0x0) ||
				    ((aktMemo.volumSlideValue>>4)!=0x0 && (aktMemo.volumSlideValue&0xF)==0xF) ||
				    useFastSlides)
				{
			...
			case 0x0B :			// Vibrato + Volume Slide
				...
				if (aktMemo.effektParam!=0) aktMemo.volumSlideValue = aktMemo.effektParam;
				// Fine Volume Up/Down and FastSlides
				if (((aktMemo.effektParam>>4)==0xF && (aktMemo.effektParam&0xF)!=0) ||
				    ((aktMemo.effektParam>>4)!=0 && (aktMemo.effektParam&0xF)==0xF) ||
				    useFastSlides)
				{
					doVolumeSlideEffekt(aktMemo);
				}
	...
	protected void doVolumeSlideEffekt(ChannelMemory aktMemo)
	{
		...
		{
			if (x==0xF && y!=0)
				aktMemo.currentSetVolume = aktMemo.currentVolume -= y;
			else
				aktMemo.currentSetVolume = aktMemo.currentVolume += x;
		}
		else
		if (y!=0)
		{
	 ...
	 * @param aktMemo
	 */
	protected void doChannelVolumeSlideEffekt(ChannelMemory aktMemo)
	{
	...
	protected void doTickEffekts(ChannelMemory aktMemo)
	{
		...
		switch (aktMemo.effekt)
		{
			...
			case 0x0C :			// Porta to Note + VolumeSlide
				...
				break;
			case 0x0E :			// Channel Volume Slide
				if (!((aktMemo.effektParam>>4)==0xF && (aktMemo.effektParam&0xF)!=0x0) || 
					 ((aktMemo.effektParam>>4)!=0x0   && (aktMemo.effektParam&0xF)==0xF))
				{
					doChannelVolumeSlideEffekt(aktMemo);
				}
				break;
